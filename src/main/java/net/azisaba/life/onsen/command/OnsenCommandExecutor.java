...
public class OnsenCommandExecutor implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("select")) {
            if (args.length == 1) {
                if (onsenManager.isSelected(playerId)) {
                    ...
                    sendMessage(player, "&f温泉の選択を解除しました");
                } else {
        ...
        if (args[0].equalsIgnoreCase("info")) {
            ...
            player.sendMessage(ChatColor.translateAlternateColorCodes('&',"&6説明文: &f機能未実装"));
            ...
            if (player.hasPermission("onsen.command.list")) {
                ...
                player.sendMessage(ChatColor.translateAlternateColorCodes('&',"&6ワールド: &f" + world));
            }
        ...
        if (args[0].equalsIgnoreCase("requests")) {
            ...
            if (args[1].equalsIgnoreCase("accept")) {
                ...
                if (args.length >= 3) {
                    ...
                    onsenName = onsenManager.getSelectedOnsen(player.getUniqueId());
                    if (onsenName == null) {
                        sendMessage(player, "&c温泉名が入力されていません！ /onsen select で選択するか、温泉名を直接入力してください");
                        sendMessage(player, "&7例: &a/onsen requests accept <温泉名>");
                        return true;
                    }
                }
            ...
            if (args[1].equalsIgnoreCase("deny")) {
                ...
                if (args.length >= 3) {
                    ...
                    if (onsenName == null) {
                        sendMessage(player, "&c温泉名が入力されていません！ /onsen select で選択するか、温泉名を直接入力してください");
                        sendMessage(player, "&7例: &a/onsen requests deny <温泉名>");
                        return true;
                    }
                }
        ...
        if (args[0].equalsIgnoreCase("register")) {
            if (args.length == 1) {
                sendMessage(player, "&b/onsen register <温泉名>");
            }
            if (args.length == 2) {
                if (args[1].equalsIgnoreCase(args[1])) {
                    String path = "OnsenList." + args[1];
        ...
        if (args[0].equalsIgnoreCase("delete")) {
            if (args.length == 1) {
                sendMessage(player, "&b/onsen delete <温泉名>");
                return true;
            }
            if (args.length == 2) {
                if (args[1].equalsIgnoreCase(args[1])) {
    ...
    private void showOnsenList(Player player, int page) {
        ...
        sendMessage(player, "&d登録されている温泉一覧");
        ...
        for (int i = start; i < end; i++) {
            ...
            if (playerUuidString.equalsIgnoreCase("Admin")) {
                ...
                try {
                    ...
                    sendMessage(player, "§c登録されているUUIDが無効です");
                    continue; // return だとループが止まるので、continueで次に進む
                }
    ...
    private void showRequestOnsenList(Player player, int page) {
        ...
        sendMessage(player, "&dリクエストされた温泉一覧 (&f" + unratedOnsenNames.size() + "&d件)");
